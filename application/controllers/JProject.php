...
class JProject extends CI_Controller
{
    ...
    public function index()
    {
       ...
       if (!empty($this->input->post('Search'))) {
            ...
            $data["search"] = $id;
            $n_row = $this->JProject_model->countquery($id)[0]->n_row;
            ...
            if($this->session->flashdata('search_jp') != NULL){
                ...
                $n_row = $this->JProject_model->countquery($data['search'])[0]->n_row;
                $config['total_rows'] = $n_row;
            }
            else{
                $data['search']= '';
        ...
        $data['user'] = $this->db->get_where('user', ['USERNAME' => $this->session->userdata('username')])->row_array();
        $data['jenis'] = $this->JProject_model->getPagination($data['search'], $config["per_page"], $data['page']);
